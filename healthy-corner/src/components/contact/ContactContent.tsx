...
import { CONTACT_INFO, SOCIAL_LINKS, BASE_PATH } from '@/lib/constants'
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <a href={SOCIAL_LINKS.whatsapp} target="_blank" rel="noopener noreferrer" className="group">
                <div className="premium-card p-10 bg-white hover:bg-[#25D366] transition-all duration-700 h-full flex flex-col items-center text-center shadow-xl border-b-8 border-transparent hover:border-[#128C7E]">
                  ...
                  <h3 className="text-3xl font-black text-text-main mb-4 group-hover:text-white transition-colors">{t('contact.whatsAppUs')}</h3>
                  <p className="text-text-muted group-hover:text-white/80 transition-colors font-medium mb-6">{t('contact.whatsAppDesc')}</p>
            ...
            <div className="bg-surface rounded-[3.5rem] p-12 shadow-2xl relative overflow-hidden group/socials">
              ...
              <h2 className="text-4xl font-black text-text-main mb-12 tracking-tight">{t('contact.socialCorner')} <span className="text-primary italic font-brand text-6xl">{t('contact.corner')}</span></h2>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-[3rem] p-8 md:p-10 shadow-2xl border border-secondary-dark/5 sticky top-28">
              <h2 className="text-3xl font-black text-text-main mb-8 leading-none">{t('contact.dropAMessage')} <br /><span className="text-primary italic font-brand text-5xl mt-2 block">{t('contact.message')}</span></h2>

              <form className="space-y-5">
                <div className="space-y-1.5">
                  <label className="text-xs font-black uppercase tracking-widest text-text-muted ml-1">{t('contact.fullName')}</label>
                  <input type="text" className="w-full px-6 py-4 bg-surface rounded-2xl border-none focus:ring-2 focus:ring-primary/20 outline-none transition-all font-medium" placeholder={t('contact.fullNamePlaceholder')} />
                </div>
                <div className="space-y-1.5">
                  <label className="text-xs font-black uppercase tracking-widest text-text-muted ml-1">{t('contact.phone')}</label>
                  <input type="tel" className="w-full px-6 py-4 bg-surface rounded-2xl border-none focus:ring-2 focus:ring-primary/20 outline-none transition-all font-medium" placeholder={t('contact.phonePlaceholder')} />
                </div>
                <div className="space-y-1.5">
                  <label className="text-xs font-black uppercase tracking-widest text-text-muted ml-1">{t('contact.messageLabel')}</label>
                  <textarea rows={4} className="w-full px-6 py-4 bg-surface rounded-2xl border-none focus:ring-2 focus:ring-primary/20 outline-none transition-all font-medium resize-none" placeholder={t('contact.messagePlaceholder')} />
                </div>
                ...
                <p className="text-center text-[10px] uppercase font-black tracking-widest text-text-muted opacity-50 mt-4 leading-relaxed">
                  {t('contact.responseTime')} <br /> {t('contact.responseTimeHours')}
                </p>
              </form>
            </div>
          </div>
